...
int finales(nacion *reino){  //Hay que ver los eventos
    ...
    if(reino->corrupcion>19){printf("\nEl Senado envenena tu comida para hacerse con el poder de La Embarrada."); return 1;} 
    if(reino->economia<1) {printf("\nLa pobre administración del Tesoro sume a La Embarrada en una fatal hambruna."); return 1; }
    if(reino->religion>19) {printf("\nLa influencia del Culto eclipsa al poder de la Corona."); return 1; }
    if(reino->religion<1) {printf("\nEl Grande castiga a los herejes por su falta de fe inundando La Embarrada y destruy%endola completamente.", 130);return 1; }
    if(reino->defensa<1) {printf("\nAl enterarse de la deficiente defensa de La Embarrada, el reino vecino invade tu territorio.");return 1; }
    if(reino->felicidad<1) {printf("\nUna turba furiosa entra al castillo y te lincha.");return 1; }
    return 0;
}
    ...
    if(lectura == 0){
        for(int i = 0; i<5;i++){
            if(iterador->consecuencia_a[i] != 0){
                ...
                if(i == 0) reino->economia += cambio;
                if(i == 1) reino->religion += cambio;
                if(i == 2) reino->defensa += cambio;
                if(i == 3) reino->felicidad += cambio;
                if(i == 4) reino->corrupcion += cambio;
            }
        }
    }
    if(lectura == 1){
